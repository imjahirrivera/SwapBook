...
function actualizarBaneadoUsuarioPermanente($conexion, $id, $baneado, $baneado_permanente) {
    $query_update = "UPDATE usuarios SET baneado_permanente = $1 WHERE id = $2";
    $result_update = pg_query_params($conexion, $query_update, array($baneado_permanente, $id));
    return $result_update;
}

function actualizarBaneadoUsuario($conexion, $id, $baneado, $baneado_permanente) {
    $fecha_baneo = $baneado ? date('Y-m-d', strtotime('+3 days')) : null;
    $query_update = "UPDATE usuarios SET baneado = $1, baneado_permanente = $2, fecha_baneo = $3 WHERE id = $4";
    $result_update = pg_query_params($conexion, $query_update, array($baneado, $baneado_permanente, $fecha_baneo, $id));
    return $result_update;
}
...
if(isset($_POST['baneado_permanente']) && isset($reporte)) {
    $resultado = actualizarEstadoReporte($conexion, $id, 'Completado');
     ...
     $resultado = actualizarBaneadoUsuarioPermanente($conexion, $usuario_reportado_id, 'false', 'true');
        ...
        <form action="" method="POST" class="bg-gray-300 shadow-md rounded px-8 pt-6 pb-8 mb-4">
            ...
            <div class="flex flex-wrap -mx-4 mb-4">
                ...
                <div class="w-full md:w-1/2 px-4">
                    <label for="reportado" class="block text-gray-700 text-sm font-bold mb-2">Nombre del Reportado</label>
                    <input type="email" name="reportado" id="reportado" class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline" value="<?php echo $reporte['usuario_reportado']; ?>" readonly>
                </div>
                <div class="w-full md:w-1/2 px-4">
                    <label for="descripcion" class="block text-gray-700 text-sm font-bold mb-2">Descripcion detallada</label>
                    <input type="text" name="descripcion" id="descripcion" class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline" value="<?php echo $reporte['descripcion']; ?>" readonly>
                </div>
                <div class="w-full md:w-1/2 px-4">
                    <label for="fecha" class="block text-gray-700 text-sm font-bold mb-2">Fecha</label>
                    <input type="text" name="fecha" id="fecha" class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline" value="<?php echo $reporte['fecha']; ?>" readonly>
                </div>
               <div class="w-full md:w-1/2 px-4">
                    <label for="libro" class="block text-gray-700 text-sm font-bold mb-2">Imagen de libro</label>
                    <?php if (!empty($reporte['foto_libro'])) { ?>
                        <img src="../<?php echo $reporte['foto_libro']; ?>" alt="Imagen del libro" class="rounded-lg">
                    <?php } else { ?>
                        <p>No se adjuntó ninguna imagen.</p>
                    <?php } ?>
                </div>
