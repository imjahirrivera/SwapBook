...
function iniciarSesion($codigo_estudiante, $contrasena) {
    ...
    if ($row && password_verify($contrasena, $row['contraseña'])) {
        ...
        if ($row['baneado'] == "t" && $row['baneado_permanente'] == "f") {
            if (strtotime($row['fecha_baneo']) < time()) {
                // El baneo ha expirado, actualizar el estado del usuario para quitar el baneo
                $query_update = "UPDATE usuarios SET baneado = 'f', fecha_baneo = NULL WHERE id = $1";
                pg_query_params($conexion, $query_update, array($row['id']));
                
                // Actualizar los datos del usuario después de quitar el baneo
                $result = pg_query_params($conexion, $query, array($codigo_estudiante));
                $row = pg_fetch_assoc($result);
            } else {
                
                header("Location: /Libros/user/baneado.php?id=" . $row['id'] . "&fecha_baneo=" . urlencode($row['fecha_baneo']));
                exit;
            }
        }

        // Verificar si el usuario está baneado permanentemente
        if ($row['baneado_permanente'] == "t" ) {
        ...
        switch ($row['estado']) {
            case 'usuario no registrado':
                header("Location: /Libros/admin/usuario_no_verificado.html");
                break;
            case 'usuario registrado':
                header("Location: /Libros/user/index_user.php?id=" . $codigo_estudiante . "&url=" . $row['foto_perfil']);
                break;
            case 'administrador':
                header("Location: /Libros/admin/listaValidaciones.php");
                break;
            case 'usuario rechazado':
                header("Location: /Libros/admin/usuario_rechazado.html");
                break;
        }
        exit;
    } else {
