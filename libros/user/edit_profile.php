...
<div id="body" class="p-10 bg-local bg-cover bg-no-repeat" style="background-image: url(../img/library.jpg);">
    ...
    <div class="mx-auto w-full max-w-screen-xl p-20 py-6 lg:py-8 bg-black/90 rounded-3xl flex flex-col">
        ...
        <div class="md:flex md:justify-between items-center">
            <span class="text-6xl font-semibold whitespace-nowrap dark:text-white">Editar Perfil</span>
        </div>

        <!-- IMAGEN, INFORMACION Y BOTON DE EDITAR PERFIL -->
        <div class="flex pt-10 pb-10 items-center">

            <!-- INFORMACION DEL PERFIL -->
            <div class="flex w-full items-center justify-center">
                <form action="cox_edit_profile.php" method="post" class="flex w-full justify-center" id="formulario" enctype="multipart/form-data">
                    <!-- Div que representa la foto de perfil y el boton de Editar perfil -->
                    <div class="flex items-center justify-center flex-col w-96 h-96 ">
                        <label class="flex flex-col w-full h-full rounded-full items-center justify-center border-2 border-gray-300 border-dashed cursor-pointer hover:bg-bray-800 bg-gray-700 hover:bg-gray-100 border-gray-600 hover:border-gray-500 hover:bg-gray-600" style="background-image: linear-gradient(rgba(0, 0, 0, 0.3), rgba(0, 0, 0, 0.3)), url('<?php echo '.' . $row -> foto_perfil?>'); background-size: cover; background-position: center;">
                            <div class="flex flex-col items-center justify-center pt-5 pb-6">
                                <div id="previsualizacion" class="mb-4"></div>
                                <p class="mb-2 text-sm text-white"><span class="font-semibold">Click para agregar tu imagen</span></p>
                                <p class="text-xs text-white">PNG o JPG</p>
                            </div>
                    ...
                    <div class="w-1/2 pl-12">
                        <label for="helper-text" class=" mb-1 text-sm font-medium text-gray-500">Nombre completo</label>
                        <input type="email" id="" class=" w-full p-1 border-b-2 border-gray-200 bg-black/90 text-white focus:border-blue-500 focus:outline-none" value="<?php echo $row->nombre_completo?>" readonly>
                        <label for="helper-text" class=" mb-1 text-sm font-medium text-gray-500 pt-6">Codigo de estudiante</label>
                        <input type="email" name="codigo_estudiante" class=" w-full p-1 border-b-2 border-gray-200 bg-black/90 text-white focus:border-blue-500 focus:outline-none" value="<?php echo $row -> codigo_estudiante?>" readonly>
                        <label for="helper-text" class=" mb-1 text-sm font-medium text-gray-500 pt-6">Correo electrónico</label>
                        <input type="email" id="" class=" w-full p-1 border-b-2 border-gray-200 bg-black/90 text-white focus:border-blue-500 focus:outline-none" value="<?php echo $row -> correo ?>" readonly>
                        <label for="helper-text" class=" mb-1 text-sm font-medium text-gray-500 pt-6">Número telefónico</label> <label for="helper-text" class=" mb-1 text-sm font-medium text-red-500 pt-6" id="error_telefono"></label>
                        <input type="number" id="telefono" name="telefono" inputmode="numeric" pattern="[0-9]+" class=" w-full p-1 border-b-2 border-gray-200 bg-black/90 text-white focus:border-blue-500 focus:outline-none" value="<?php echo $row -> telefono?>">
                        <label for="helper-text" class=" mb-1 text-sm font-medium text-gray-500 pt-6">Contraseña</label> <label for="helper-text" class=" mb-1 text-sm font-medium text-red-500 pt-6" id="error_contrasena"></label>
                        <input type="text" id="contrasena" name="contrasena" class=" w-full pt-1 border-b-2 border-gray-200 bg-black/90 text-white focus:border-blue-500 focus:outline-none" value="" placeholder="Ingresa una nueva contraseña">
                    </div>
                </form>
            </div>
        </div>
        ...
        <div class="flex items-center items-center justify-center">
            <button type="button" onclick="enviarFormulario();" class="ml-4 pl-4 pr-4 relative flex w-fit h-12 rounded-full bg-white focus:ring-2 focus:ring-white focus:ring-offset-2 focus:ring-offset-gray-800 items-center justify-center mt-auto hover:bg-gray-200" id="user-menu-button" aria-expanded="false" aria-haspopup="true">
                <span class="text-xl font-semibold whitespace-nowrap dark:text-black">Guardar Perfil</span>
            </button>
        </div>
...
function enviarFormulario() {
    ...
    if(telefono.length != 10){
        ...
        error_telefono.textContent = "❌Formato de telefono no valido.";
        return;
    }
    ...
    if((contrasena.length > 0) && (contrasena.length < 8)){
        ...
        error_contrasena.textContent = "❌Contraseña  invalida: 8 caracteres o más.";
        return;
    }
...
<script>  
    function cambiarArchivo(event, idPrevisualizacion) {
        ...
        lector.onload = function(event) {
            ...
            imagen.style.width = '24rem'; // Asegura que la imagen ocupe todo el contenedor
            imagen.style.height = '24rem'; // Asegura que la imagen ocupe todo el contenedor
            imagen.style.display = "flex";
            imagen.style.marginTop = "62px";
            imagen.style.justifyContent = 'center'; // Centra horizontalmente
            imagen.style.alignItems = 'center'; // Centra verticalmente
            imagen.style.objectFit = 'cover'; // Asegura que la imagen se recorte para ajustarse al contenedor
            imagen.style.borderRadius = '50%'; // Hace la imagen circular
