...
$queryActual = "SELECT * FROM usuarios WHERE codigo_estudiante = '$codigo_estudiante';";
$resultActual = pg_query($conexion, $queryActual);
$actual = pg_fetch_object($resultActual);
?>
...
<div id="body" class="p-10 bg-local bg-cover bg-no-repeat" style="background-image: url(../img/library.jpg);">
    ...
    <div class="mx-auto w-full max-w-screen-xl p-20 py-6 lg:py-8 bg-black/90 rounded-3xl flex flex-col">
        ...
        <div class="flex justify-center flex-col">
            <span class="text-4xl font-semibold whitespace-nowrap text-white">Reportar a este usuario</span>
            <label class=" mb-1 text-base font-medium text-gray-300 mt-2">Recuerda lo siguiente:</label>
            <ul class="ml-8 list-disc text-white">
                <li><span class="text-sm text-gray-300">El reportar a un usuario puede finalizar en la suspensión temporal o permanente del usuario.</span></li>
                <li><span class="text-sm text-gray-300">Al mandar este reporte afirmas que todo lo que está escrito por ti es verídico y tienes manera de comprobarlo.</span></li>
                <li><span class="text-sm text-gray-300">El mal uso de la herramienta REPORTES puede finalizar en la suspensión permanente de tu cuenta.</span></li>
                <li><span class="text-sm text-gray-300">SwapBook no está obligado a darle una solución material, monetaria o de penalización a tu caso.</span></li>
            </ul>
        </div>
        <!-- IMAGEN, INFORMACION Y BOTON DE EDITAR PERFIL -->
        <div class="flex mt-5 mb-10 justify-center flex-col ">
            ...
            <div class="flex w-full items-center justify-center">
                <form action="<?php echo 'cox_reporte.php?id=' . $_GET['id'] . '&url=' . $_GET['url'] . '&id_usuario=' . $_GET['id_usuario'];?>" method="POST" class="flex flex-col items-center w-full justify-center" id="formulario" enctype="multipart/form-data">
                    <div class="flex flex-col w-1/2">
                        <div class="pb-6">
                            <label class=" mb-1 text-base font-medium text-gray-300 pt-6">Razón del reporte</label> <label class=" mb-1 text-sm font-medium text-red-500 pt-6 pl-2" id="error_resena"> </label>
                            <textarea id="mensaje" name="mensaje" rows="4" class=" w-full h-48 p-1 border-b-2 border-gray-200 bg-black/90 text-white focus:border-blue-500 focus:outline-none" style="resize: none;" placeholder="Descripción. Max. 200 caracteres."></textarea>
                        </div>
                        <div class="flex items-center justify-center flex-col w-full">
                            <div class="w-full">
                                <label class="text-base font-medium text-gray-300 pt-6">Evidencia</label> <label class="whitespace-nowrap mb-1 text-sm font-medium text-red-500 pt-6 pl-2" id="error_img"> </label><br>
                                <label class="text-sm font-medium text-gray-500 pt-6">Trata de ser lo más claro posible con tu evidencia, que se vea claro la razón del reporte. (1 sola imagen)</label>
                            </div>
                            <label class="my-2 flex flex-col items-center justify-center w-full h-3/4 border-2 border-gray-300 border-dashed rounded-lg cursor-pointer bg-gray-50 dark:hover:bg-bray-800 dark:bg-gray-700 hover:bg-gray-100 dark:border-gray-600 dark:hover:border-gray-500 dark:hover:bg-gray-600">
        ...
        <div class="flex items-center items-center justify-center">
            <button type="submit" onclick="reportar();" class="ml-4 pl-4 pr-4 relative flex w-36 h-16 rounded-full bg-red-500 focus:ring-2 focus:ring-white focus:ring-offset-2 focus:ring-offset-gray-800 items-center justify-center mt-auto" id="user-menu-button" aria-expanded="false" aria-haspopup="true">
                <span class="text-xl font-semibold whitespace-nowrap text-white">Reportar</span>
            </button>
        </div>
...
<script>
    ...
    function reportar() {
        document.getElementById('error_resena').textContent = "";
        document.getElementById('error_img').textContent = "";

        let mensaje = document.getElementById('mensaje').value;
        let img = document.getElementById('foto_libro').value;

        let error = false;

        if(mensaje == "") {
            document.getElementById('error_resena').textContent = "❌No puedes dejar este espacio vacío.";
            error = true;
        }

        if(img == "") {
            document.getElementById('error_img').textContent = "❌No puedes dejar este espacio vacío.";
            error = true;
        }
